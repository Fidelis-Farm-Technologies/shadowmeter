                             ...
                             MMDB_s *country_mmdb)
{
    ...
    duckdb_append_uint64(appender, flow_id);
    ...
    uint32_t sasn = 0;
    uint32_t dasn = 0;
    char sasnorg[ASNORG_LEN] = {0};
    char dasnorg[ASNORG_LEN] = {0};
    if (asn_mmdb)
    {
        result =
...
static int process_yaf_record(uint64_t flow_id,
                              const char *observation,
                              ...
                              MMDB_s *country_mmdb)
{
    ...
    if (append_yaf_record(appender, flow_id, observation, ndpi_ctx, flow, asn_mmdb, country_mmdb) < 0)
    {
        return -1;
    }
...
static uint64_t yaf_epoch_micro()
{
    struct timeval tv;
               ...
               const char *country_file)
{
    ...
    // fprintf(stderr, "asn_file: %s\n", asn_file);
    // fprintf(stderr, "country_file: %s\n", country_file);
    ...
    {
        if (process_yaf_record(flow_id, observation, appender, ndpi_ctx, &yaf_record, asn_mmdb_ptr, country_mmdb_ptr) < 0)
        {
    ...
    printf("%s: processed %s [%lu records]\n", __FUNCTION__, input_file, flow_id);
